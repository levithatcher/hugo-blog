...
d %>% 
  ...
  mutate(Mean = base::mean(TMin, na.rm = TRUE),
         ...
         StdError = stats::sd(TMin, na.rm = TRUE) / n(),
         CI = 1.96 * StdError) %>% 
  
  ggplot(., aes(x = Mean, y = Decade)) +
    geom_point() +
    geom_errorbarh(aes(xmin = Mean - CI, xmax = Mean + CI)) +
    scale_y_continuous(breaks = c(1940, 1950, 1960, 1970, 1980, 1990, 2000, 2010)) +
