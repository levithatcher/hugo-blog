...
```{r}
# library(ggseas)
# new <- ts(df_slim_hourly$Measurement, start=c(2013,1), frequency = 8760)
# df_new <- tsdf(new)
# 
# ggplot(data = df_new, aes(x = x, y = y)) +
#   geom_point() +
#   stat_seas(color = "red", size = 3) +
#   #stat_stl(s.window = 8760, colour = "red", size = 3) +
#   #labs(title = "PM2.5 in SLC from 2013 - 2017") +
#   scale_y_log10(name = "PM2.5 µg/m3", limits = c(1,NA), breaks = c(1,20,40,60,80,100))
#   #scale_x_datetime(name = "Year", labels=date_format("%m-%Y"), breaks = date_breaks("1 year"))

# ggsdc(data = df_slim_hourly, aes(x = DateTime, y = Measurement), method = "seas") + 
#   geom_line()
```

Let's go back and grab the daily data (instead of hourly)

```{r}
# Grab only daily readings, remove readings of PM2.5 below 0, and grab only needed columns
df_slim_daily <- df %>%
  filter(`Sample Duration` == '24 HOUR' & Measurement >= 0) %>%
  arrange(DateTime) 
  select(DateTime, Measurement)

df_slim_daily$DateTime <- as.Date(df_slim_daily$DateTime)
...
Let's first plot the daily PM2.5 data

```{r}
library(ggplot2)
library(scales)
ggplot(data = df_slim_daily, aes(x = DateTime, y = Measurement)) +
  geom_line() +
  labs(title = "Daily PM2.5 in SLC from 2013 - 2017") +
  scale_y_log10(name = "PM2.5 µg/m3", limits = c(1,NA), breaks = c(1,20,40,60,80,100)) +
  scale_x_date(name = "Year", labels=date_format("%m-%Y"), breaks = date_breaks("1 year"))
```
...
measurement_ts <- ts(measurement_vect_daily, start=c(2013,1,1), frequency = 365.25)

# Perform decomposition into parts via additive component
stl_slim <- stats::stl(x = measurement_ts, s.window = "periodic")


# Prep trend data so we can plot with raw data from above
# What's the standard way to do use ts objects in ggplot2?
library(ggseas)
df_trend <- tsdf(stl_slim$time.series[,'trend'])
df_trend$DateTime <- df_slim_daily$DateTime

ggplot(df_trend, aes(x = DateTime, y = y)) +
   geom_line(colour = "red", size = 3) +
   stat_seas() 
   # geom_line(data = df_slim_daily, colour = "black", aes(x = DateTime, y = Measurement)) +
   # labs(title = "PM2.5 in SLC from 2013 - 2017") +
   # scale_y_log10(name = "PM2.5 µg/m3", limits = c(1,NA), breaks = c(1,20,40,60,80,100))
   #scale_x_continuous(name = "Year", labels=date_format("%m-%Y"), breaks = date_breaks("1 year"))

# ggplot(data = df_trend, aes(x = DateTime, y = y)) +
#   geom_line() +
#   labs(title = "PM2.5 in SLC from 2013 - 2017")
  #scale_y_log10(name = "PM2.5 µg/m3", limits = c(1,NA), breaks = c(1,20,40,60,80,100)) +
  #scale_x_datetime(name = "Year", labels=date_format("%m-%Y"), breaks = date_breaks("1 year"))
