...
Reduce tibble to only desired columns and code DateTime column

```{r}
df$DateTime <- as.POSIXct(paste(df$Date.Local, df$X24.Hour.Local), 
                          format="%Y-%m-%d %H:%M")
#df_slim <- df %>% select(DateTime, Sample.Measurement)
#df_slim
```
...
library(ggplot2)
library(scales)
ggplot(data=df_slim, aes(x = DateTime, y = Sample.Measurement)) +
  geom_point() +
...
The trend looks like it's negative, right? That's engouraging! Anecdotally in SLC, each winter seems as bad as the last (perhaps because the issue is receving more attention each year).

What exactly is the trend?
```{r}
eq = lm(Sample.Measurement ~ as.Date(DateTime), data = df_slim)
print(eq$coefficients)
```

When converting to a yearly rate of change (i.e., by multiplying the coefficient by 365) we see roughly a `print(eq$coefficients[2])` µg/m3 decrease per year in PM2.5.
