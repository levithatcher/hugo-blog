...
```{r}
library(dplyr)
df <- read.csv('../data/SLCHawthornePM2-5_2013-2017.csv')
df
```
...
```{r}
library(dplyr)
df$DateTime <- as.POSIXct(paste(df$Date.Local, df$X24.Hour.Local), format="%Y-%m-%d %H:%M")
df_slim <- select(df, DateTime, Sample.Measurement)
df_slim
```
...
```{r}
lm(Sample.Measurement ~ (DateTime * 3600 * 365), data = df_slim)
```
